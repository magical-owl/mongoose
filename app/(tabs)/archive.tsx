...
export default function ArchiveScreen() {
  ...
  const sectionItems: { id: ArchiveSection; label: string }[] = [
    { id: 'chapters', label: 'Chapters' }, { id: 'rituals', label: 'Rituals' }, { id: 'collections', label: 'Collections' }, { id: 'replay', label: 'Replay' }, { id: 'garden', label: 'Garden' },
  ];

  return (
    <View style={[styles.root, { backgroundColor: theme.colors.background }]}>
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 16, paddingHorizontal: 20, paddingBottom: insets.bottom + 80 }} showsVerticalScrollIndicator={false}>
        <Text style={[styles.title, { color: theme.colors.text }]}>🗂️ Personal Archive</Text>
        <Text preset="bodySmall" color="textSecondary" style={{ marginBottom: 16 }}>Shape your journal around the parts of life that matter to you.</Text>
        ...
        {section === 'replay' && <><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabs}>{(['same-date', 'same-month', 'one-year'] as ReplayMode[]).map((mode) => <TouchableOpacity key={mode} onPress={() => setReplayMode(mode)} style={[styles.tab, { borderColor: replayMode === mode ? theme.colors.tint : theme.colors.border, backgroundColor: replayMode === mode ? theme.colors.tint : theme.colors.surface }]}><Text preset="caption" style={{ color: replayMode === mode ? '#fff' : theme.colors.text }}>{mode === 'same-date' ? 'Same date' : mode === 'same-month' ? 'Same month' : 'One year ago'}</Text></TouchableOpacity>)}</ScrollView>{replay.length === 0 ? <Text preset="body" color="textSecondary">No memories match this replay yet.</Text> : replay.map((entry) => <View key={entry.id} style={[styles.card, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}><Text style={styles.cardIcon}>{entry.sentiment?.mood ? getMoodEmoji(entry.sentiment.mood) : '📜'}</Text><View style={{ flex: 1 }}><Text preset="label" color="text">{entry.title}</Text><Text preset="caption" color="textSecondary">{entry.date} · {entry.manualMoodWeather}</Text></View></View>)}</>}
        ...
        {section !== 'replay' && section !== 'garden' && ((section === 'chapters' && extras.chapters.length === 0) || (section === 'rituals' && extras.rituals.length === 0) || (section === 'collections' && extras.collections.length === 0)) && <Text preset="body" color="textSecondary">Nothing here yet. Add your first one above.</Text>}
      </ScrollView>
    </View>
  );
}

const styles = StyleSheet.create({ root: { flex: 1 }, title: { fontSize: 24, fontWeight: '700', marginBottom: 4 }, tabs: { gap: 8, paddingBottom: 16 }, tab: { borderWidth: 1, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8 }, addRow: { flexDirection: 'row', gap: 8, marginBottom: 16 }, input: { flex: 1, borderWidth: 1, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10 }, addButton: { borderRadius: 8, paddingHorizontal: 16, justifyContent: 'center' }, card: { borderWidth: 1, borderRadius: 10, padding: 14, marginBottom: 10, flexDirection: 'row', alignItems: 'center', gap: 12 }, cardIcon: { fontSize: 28 }, completeButton: { borderRadius: 8, paddingHorizontal: 10, paddingVertical: 7 }, garden: { borderWidth: 1, borderRadius: 12, padding: 24, alignItems: 'center' }, bookCard: { borderWidth: 1, borderRadius: 10, padding: 14, marginTop: 12 }, gardenIcon: { fontSize: 58 }, gardenStats: { fontSize: 18, fontWeight: '700', marginTop: 24 }, });
