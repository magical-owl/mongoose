...
import { manualMoodWeatherLabel, useTranslation } from '@/localization/i18n';
    ...
    <View style={[styles.root, { backgroundColor: theme.colors.background }]}>
      ...
      <ScrollView contentContainerStyle={{ paddingTop: 4, paddingHorizontal: 20, paddingBottom: insets.bottom + 80 }} showsVerticalScrollIndicator={false}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabs}>
          {sectionItems.map((item) => <TouchableOpacity key={item.id} onPress={() => setSection(item.id)} style={[styles.tab, { borderColor: section === item.id ? theme.colors.tint : theme.colors.border, backgroundColor: section === item.id ? theme.colors.tint : theme.colors.surface }]}><Text preset="caption" style={{ color: section === item.id ? theme.colors.background : theme.colors.text }}>{item.label}</Text></TouchableOpacity>)}
        </ScrollView>
        ...
        {section !== 'replay' && section !== 'bin' && (
          <View style={styles.addRow}>
            ...
            <TouchableOpacity onPress={() => { void add(); }} style={[styles.addButton, { backgroundColor: theme.colors.tint }]}><Text preset="label" style={{ color: theme.colors.background }}>Add</Text></TouchableOpacity>
          </View>
        )}
        ...
        {section === 'replay' && <><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabs}>{(['same-date', 'same-month', 'one-year'] as ReplayMode[]).map((mode) => <TouchableOpacity key={mode} onPress={() => setReplayMode(mode)} style={[styles.tab, { borderColor: replayMode === mode ? theme.colors.tint : theme.colors.border, backgroundColor: replayMode === mode ? theme.colors.tint : theme.colors.surface }]}><Text preset="caption" style={{ color: replayMode === mode ? theme.colors.background : theme.colors.text }}>{mode === 'same-date' ? 'Same date' : mode === 'same-month' ? 'Same month' : 'One year ago'}</Text></TouchableOpacity>)}</ScrollView>{replay.length === 0 ? <Text preset="body" color="textSecondary">No memories match this replay yet.</Text> : replay.map((entry) => <TouchableOpacity key={entry.id} onPress={() => router.push(`/entry/${entry.id}`)} style={[styles.card, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>{getEntryManualMoods(entry).length > 0 ? <MoodBadgeList moods={getEntryManualMoods(entry)} maxVisible={2} compact style={styles.replayMoodBadges} /> : <Text style={styles.cardIcon}>📜</Text>}<View style={{ flex: 1 }}><Text preset="label" color="text">{entry.title}</Text><Text preset="caption" color="textSecondary">{entry.date} · {manualMoodWeatherLabel(entry.manualMoodWeather, t)}</Text></View><Text color="textSecondary">›</Text></TouchableOpacity>)}</>}
...
const styles = StyleSheet.create({ root: { flex: 1 }, fixedHeader: { zIndex: 30, elevation: 30, paddingHorizontal: 20 }, hero: { borderWidth: 1, borderRadius: 12, padding: 16, marginBottom: 16 }, eyebrow: { fontWeight: '700', letterSpacing: 1, marginBottom: 6 }, title: { fontSize: 24, fontWeight: '700', marginBottom: 16 }, stats: { flexDirection: 'row', gap: 18, marginTop: 16 }, tabs: { gap: 8, paddingBottom: 16 }, tab: { borderWidth: 1, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8 }, addRow: { flexDirection: 'row', gap: 8, marginBottom: 16 }, input: { flex: 1, borderWidth: 1, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10 }, addButton: { borderRadius: 8, paddingHorizontal: 16, justifyContent: 'center' }, card: { borderWidth: 1, borderRadius: 10, padding: 14, marginBottom: 10, flexDirection: 'row', alignItems: 'center', gap: 12 }, cardIcon: { fontSize: 28 }, replayMoodBadges: { maxWidth: 188 }, completeButton: { borderRadius: 8, paddingHorizontal: 10, paddingVertical: 7 }, binCard: { alignItems: 'flex-start' }, binActions: { flexDirection: 'row', alignItems: 'center', gap: 8 }, binButton: { minHeight: 32, borderWidth: 1, borderRadius: 8, paddingHorizontal: 10, alignItems: 'center', justifyContent: 'center' }, binButtonText: { fontWeight: '800' }, garden: { borderWidth: 1, borderRadius: 12, padding: 24, alignItems: 'center' }, bookCard: { borderWidth: 1, borderRadius: 10, padding: 14, marginTop: 12 }, gardenIcon: { fontSize: 58 }, gardenStats: { fontSize: 18, fontWeight: '700', marginTop: 24 }, });
